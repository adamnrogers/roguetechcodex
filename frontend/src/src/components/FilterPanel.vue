  ...
  <aside class="filter-panel">
    <div class="filter-section">
      <h3 class="filter-title">
        {{ mode === 'battle_armor' ? 'Tonnage' : 'Class &amp; Tonnage' }}
      </h3>
      <template v-if="mode === 'battle_armor'">
        <label v-for="t in BA_TONNAGES" :key="t.value" class="filter-check">
          <input
            type="checkbox"
            :checked="tonnage.includes(t.value)"
            @change="toggleTonnage(t.value)"
          />
          {{ t.label }}
        </label>
      </template>
        ...
        <div v-for="group in activeGroups" :key="group.key" class="tg-group">
          ...
          <div v-if="expanded[group.key]" class="tg-options">
            <label v-for="opt in group.options" :key="opt.label" class="filter-check tg-opt">
              <input type="checkbox" :checked="isOptionChecked(opt)" @change="toggleOption(opt)" />
              {{ opt.label }}
            </label>
          </div>
        </div>
    ...
    <div class="filter-section">
      <label class="filter-check">
        <input
          type="checkbox"
          :checked="uniqueOnly"
          @change="emit('update:uniqueOnly', !uniqueOnly)"
        />
        Unique only
      </label>
    </div>
    <div v-if="mode === 'mech'" class="filter-section">
      ...
      <label class="filter-check">
        <input
          type="checkbox"
          :checked="!!hasLowerArm"
          @change="emit('update:hasLowerArm', hasLowerArm ? null : true)"
        />
        Lower Arm
      </label>
      <label class="filter-check">
        <input
          type="checkbox"
          :checked="!!hasHand"
          @change="emit('update:hasHand', hasHand ? null : true)"
        />
        Hand
      </label>
    </div>
    <div v-if="mode === 'mech'" class="filter-section hardpoints-section">
      ...
      <div class="hp-grid">
        ...
        <div
          v-for="row in HP_TYPES"
          :key="row.key"
          class="hp-row"
          :class="{ 'hp-row-dim': hardpoints[row.key].count === 0 }"
        >
      ...
      <div class="hp-omni-toggles">
        <label class="filter-check hp-excl-omni">
          <input
            ...
            @change="
              emit('update:hardpoints', {
                ...hardpoints,
                excludeOmni: !hardpoints.excludeOmni,
                omniOnly: false,
              })
            "
          />
...
const VTOL_GROUPS: TonnageGroup[] = [
  ...
  { key: 'heavy', label: 'Heavy', sublabel: '60t', options: [{ label: '60t', values: [60] }] },
]
...
function toggleGroup(group: TonnageGroup) {
  ...
  if (isGroupChecked(group)) {
    emit(
      'update:tonnage',
      props.tonnage.filter((v) => !vals.includes(v)),
    )
  } else {
...
function toggleOption(opt: TonnageOption) {
  if (isOptionChecked(opt)) {
    emit(
      'update:tonnage',
      props.tonnage.filter((v) => !opt.values.includes(v)),
    )
  } else {
